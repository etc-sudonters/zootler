...
func main() {
	...
	compileEnv := mido.NewCompileEnv(
		...
		func(env *mido.CompileEnv) {
			...
			for i := range rawTokens {
				...
				ptr := objects.PackPtr32(objects.PtrToken, uint32(i))
				env.Objects.AssociateSymbol(symbol, ptr)
			}
			for i, name := range settings.Names() {
				symbol := env.Symbols.LookUpByName(name)
				env.Objects.AssociateSymbol(symbol, objects.PackPtr32(objects.PtrSetting, uint32(i)))
			}
		},
	)
	...
	for i := range source {
		...
		if declaration.Kind == mido.SourceEvent {
			...
			ptr := objects.PackPtr32(objects.PtrToken, 0xdeadbeef)
			compileEnv.Objects.AssociateSymbol(symbol, ptr)
		}
	}
