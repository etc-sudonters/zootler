...
func ReadTape(tape tape) string {
	...
	for idx < end {
		...
		case TEMP_IA_LOAD_STR:
			...
			idx += 2
			break
		case TEMP_IA_PRODUCE_EQ:
			tt.padToGutter(0)
			tt.WriteString("==")
			break
		case TEMP_IA_PRODUCE_NQ:
			tt.padToGutter(0)
			tt.WriteString("!=")
			break
		case TEMP_IA_PRODUCE_LT:
			tt.padToGutter(0)
			tt.WriteString("<")
			break
		case TEMP_IA_INVERT:
...
func (t *tape) writeLoadString(handle uint16) int {
	return t.write(TEMP_IA_LOAD_STR, encodeU16(handle)...)
}
...
func (t *tape) writeReduceAnd() int {
	return t.write(IA_REDUCE_ALL)
}

func (t *tape) writeReduceOr() int {
	return t.write(IA_REDUCE_ANY)
}
