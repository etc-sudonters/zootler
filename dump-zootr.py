...
def main(args: argparse.Namespace) -> int:
    ...
    dump_to_file("items", output / "data" / "items.json", list(ItemInfo.items.values()))
    dump_to_file(
        "locations",
    ...
    copy_logic_dir(zootr / "data" / "Glitched World", output / "logic" / "glitched")
    copy_file(zootr / "data" / "LogicHelpers.json", output / "logic" / "helpers.json")
    return 0

def cli() -> argparse.ArgumentParser:
    cli = argparse.ArgumentParser("zootr-dump", description="Dumps data from OOT-Randomizer")
    cli.add_argument(
        "-Z",
        "--zootr",
...
def rearrange_location_table(tbl) -> Any:
    return [
        {"name": name, **dict(zip(_LOCATION_TABLE_HEADERS, data))}
