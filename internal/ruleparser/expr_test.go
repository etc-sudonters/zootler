...
func (v ValidatingVisitor) unexpected(expected Tree) error {
	return fmt.Errorf("\nExpected\n%+v\n\nActual\n%+v", expected, v.parsed)
}
...
func (v ValidatingVisitor) VisitBinOp(o *BinOp) error {
	...
	err := VisitOld(ValidatingVisitor{e.Left}, o.Left)
	if err != nil {
		return v.trace(err)
	}
	err = VisitOld(ValidatingVisitor{e.Right}, o.Right)
	if err != nil {
		return v.trace(err)
	}
...
func (v ValidatingVisitor) VisitCall(o *Call) error {
	...
	err := VisitOld(ValidatingVisitor{e.Callee}, o.Callee)
	if err != nil {
		return v.trace(err)
	}

	for i := range o.Args {
		err = VisitOld(ValidatingVisitor{e.Args[i]}, o.Args[i])
		if err != nil {
			return v.trace(err)
		}
	}
...
func (v ValidatingVisitor) VisitSubscript(o *Subscript) error {
	...
	err := VisitOld(ValidatingVisitor{e.Target}, o.Target)
	if err != nil {
		return v.trace(err)
	}

	err = VisitOld(ValidatingVisitor{e.Index}, o.Index)
	if err != nil {
		return v.trace(err)
	}
...
func (v ValidatingVisitor) VisitTuple(o *Tuple) error {
	...
	for i := range o.Elems {
		err := VisitOld(ValidatingVisitor{e.Elems[i]}, o.Elems[i])
		if err != nil {
			return v.trace(err)
		}
	}
